...
export class ModOptionsSingleton {
    save(modID, optionID, value) {
        ...
        if (localStorage.length > 1) {
            console.warn(`${modID}: ModOptions: erasing storage (${localStorage.length} items)`);
            localStorage.clear();
        }
        ...
        console.warn(`${modID}: SAVE ${optionName}=${value}`);
    }
    load(modID, optionID) {
        const optionName = `${modID}.${optionID}`;
        const value = UI.getOption("user", "Mod", optionName);
        if (value != null) {
            console.warn(`${modID}: LOAD ${optionName}=${value} (saved)`);
            return value;
        }
        ...
        try {
            ...
            console.warn(`${modID}: LOAD ${optionName}=${value} (stored)`);
            return value;
        }
        catch (e) {
            console.error(`${modID}: ModOptions: error loading options`);
